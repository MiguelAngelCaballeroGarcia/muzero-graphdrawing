...
class GraphDrawing:
    ...
    def _canonical_edges(self, edges: Iterable[Tuple[int, int]]) -> List[Tuple[int, int]]:
        """
    ...
    def plot(self, show_ids: bool = True, title: str = "", block: bool = False) -> plt.Figure:
        ...
        if absent:
            ax.scatter([((i-1) % self.n) for i in absent],
                       [((i-1) // self.n) for i in absent],
                       s=120, facecolors='none', edgecolors='gray', linewidths=1.4, zorder=2)
    
        # Node IDs (or flattened positions)
        if show_ids:
            for nid, (i, j) in self.pos.items():
                flat_pos = i * self.n + j + 1
                ax.text(j, i + 0.18, str(nid), ha='center', va='bottom', fontsize=9, color='blue', zorder=4)
    
        # Optional: label grid coordinates for debugging
        for i in range(self.n):
            for j in range(self.n):
                ax.text(j, i - 0.15, f'({i},{j})', ha='center', va='top', fontsize=7, color='gray', zorder=2)
        ...
        if self.crossings is not None:
            fig.text(0.5, 0.02, f"Edge crossings: {self.crossings}", ha='center', va='bottom', fontsize=10)
